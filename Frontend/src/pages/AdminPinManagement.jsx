// src/pages/AdminPanel.jsx - Version complète avec gestion des utilisateurs et agents
...
import { 
  ...
  FaSync, FaLock, FaUnlockAlt, FaUndo
} from 'react-icons/fa'
...
function AdminPanel({ user }) {
  ...
  const toggleUserStatus = async (userId, currentStatus, userName) => {
    ...
    if (!window.confirm(`Êtes-vous sûr de vouloir ${action} l'utilisateur "${userName}" ?`)) return
    ...
    try {
      ...
      toast.success(`Utilisateur ${action === 'bloquer' ? 'bloqué' : 'débloqué'} avec succès`)
      fetchAdminData()
  ...
  const confirmResetPassword = async () => {
    ...
    try {
      ...
      toast.success(`Mot de passe réinitialisé ! Nouveau mot de passe: ${response.data.new_password}`)
  ...
  const deleteUser = async (userId, userName) => {
    if (!window.confirm(`Êtes-vous sûr de vouloir supprimer définitivement l'utilisateur "${userName}" ? Cette action est irréversible.`)) return
  ...
  // Blocage/Déblocage d'agent
  const toggleAgentStatus = async (agentId, currentStatus, agentName) => {
    const action = currentStatus ? 'bloquer' : 'débloquer'
    if (!window.confirm(`Êtes-vous sûr de vouloir ${action} l'agent "${agentName}" ?`)) return
    
    try {
      const token = localStorage.getItem('accessToken')
      await axios.put(`/api/admin/agents/${agentId}/toggle-status`, {
        is_active: !currentStatus
      }, {
        headers: { Authorization: `Bearer ${token}` }
      })
      
      toast.success(`Agent ${action === 'bloquer' ? 'bloqué' : 'débloqué'} avec succès`)
      fetchAdminData()
  ...
  const deleteAgent = async (agentId, agentName) => {
    if (!window.confirm(`Êtes-vous sûr de vouloir supprimer définitivement l'agent "${agentName}" ? Cette action est irréversible.`)) return
  ...
  // ============================================
  // RÉINITIALISATION DE CLÉ PRIVÉE
  // ============================================
  const handleResetPrivateKey = async (userId, userType, userName, userPhone) => {
  ...
  const confirmResetKey = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.success('Clé privée réinitialisée avec succès')
      }
    } catch (error) {
  ...
  const resetUserModal = () => {
    ...
    setCreatedUser(null)
    setUserForm({
      phone: '', fullname: '', password: '', confirmPassword: '', email: '', province: '', city: '', address: '', role: 'user'
    })
  ...
  const resetAgentModal = () => {
    ...
    setAgentForm({
      phone: '', fullname: '', password: '', confirmPassword: '', province: '', agency_name: '', agency_address: '', agency_phone: '', agency_type: 'secondaire'
    })
      ...
      <div className="card">
                    ...
                    <tr key={u.id} className="border-b border-white/10">
                      ...
                      <td className="py-3 text-center">
                        <div className="flex flex-wrap gap-1 justify-center">
                          ...
                          <button
                            onClick={() => handleResetPrivateKey(u.id, u.role, u.fullname, u.phone)}
                            className="p-1.5 rounded-lg bg-orange-500/20 text-orange-400 hover:bg-orange-500/30 transition-all"
                            title="Réinitialiser clé privée"
                          >
                            <FaSync size={14} />
                          </button>
                          <button
                            onClick={() => toggleUserStatus(u.id, u.is_active, u.fullname)}
                          ...
                          >
                            {u.is_active ? <FaBan size={14} /> : <FaUnlockAlt size={14} />}
                          </button>
                          <button
                    ...
                    <tr key={a.id} className="border-b border-white/10">
                      ...
                      <td className="py-3 text-center">
                        <div className="flex flex-wrap gap-1 justify-center">
                          ...
                          <button
                            onClick={() => openResetPasswordModal({ ...a, role: 'agent' })}
                            className="p-1.5 rounded-lg bg-green-500/20 text-green-400 hover:bg-green-500/30 transition-all"
                            title="Réinitialiser mot de passe"
                          >
                            <FaUndo size={14} />
                          ...
                          <button
                            onClick={() => handleResetPrivateKey(a.id, a.role, a.fullname, a.phone)}
                            className="p-1.5 rounded-lg bg-orange-500/20 text-orange-400 hover:bg-orange-500/30 transition-all"
                            title="Réinitialiser clé privée"
                          >
                            <FaSync size={14} />
                          ...
                          <button
                            onClick={() => toggleAgentStatus(a.id, a.is_active, a.fullname)}
                            className="p-1.5 rounded-lg bg-yellow-500/20 text-yellow-400 hover:bg-yellow-500/30 transition-all"
                            title={a.is_active ? 'Bloquer' : 'Débloquer'}
                          >
                            {a.is_active ? <FaBan size={14} /> : <FaUnlockAlt size={14} />}
                          </button>
                          <button
                            onClick={() => deleteAgent(a.id, a.fullname)}
                            className="p-1.5 rounded-lg bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-all"
                            title="Supprimer"
                          >
                            <FaTrashAlt size={14} />
        ...
        {activeTab === 'blog' && (
          <div>
            ...
            {blogLoading ? (
              <div className="flex justify-center py-12"><div className="text-white/60">Chargement des articles...</div></div>
            ) : blogPosts.length === 0 ? (
        ...
        {activeTab === 'kyc' && (
          <div>
            ...
            {kycLoading ? (
              <div className="flex justify-center py-12"><div className="text-white/60">Chargement des demandes KYC...</div></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80">
          <div className="relative max-w-md w-full bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl shadow-2xl border border-green-500/30">
            <div className="flex justify-between items-center p-4 border-b border-gray-700">
              ...
              <button onClick={() => setShowResetPasswordModal(false)} className="text-gray-400 hover:text-white"><FaTimes size={18} /></button>
            </div>
            <div className="p-5">
              <p className="text-gray-300 mb-4">Utilisateur: <span className="font-bold text-white">{selectedUser.fullname}</span> ({selectedUser.phone})</p>
              <div className="mb-4"><label className="block text-sm text-gray-300 mb-1">Nouveau mot de passe</label><input type="text" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-gray-700 border border-gray-600 text-white" placeholder="Au moins 4 caractères" required /></div>
              <div className="flex gap-3"><button onClick={() => setShowResetPasswordModal(false)} className="flex-1 px-3 py-2 rounded-lg bg-gray-700 text-white">Annuler</button><button onClick={confirmResetPassword} disabled={resettingPassword} className="flex-1 px-3 py-2 rounded-lg bg-green-600 text-white flex items-center justify-center gap-2">{resettingPassword ? '...' : <><FaUndo /> Réinitialiser</>}</button></div>
            </div>
          </div>
        </div>
      )}

      {/* MODAL RÉINITIALISATION CLÉ PRIVÉE */}
      {showResetKeyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 overflow-y-auto">
          <div className="relative max-w-md w-full bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl shadow-2xl border border-orange-500/30">
            <div className="sticky top-0 bg-gray-800/95 p-4 border-b border-gray-700 flex justify-between items-center rounded-t-2xl">
            ...
            <div className="p-6">
              {resetResult ? (
                <div className="text-center">
                  <div className="inline-flex p-4 bg-green-500/20 rounded-full mb-4"><FaCheckCircle className="text-green-400 text-4xl" /></div>
                  <h2 className="text-2xl font-bold text-white mb-4">Clé réinitialisée !</h2>
                  <div className="bg-white/10 rounded-xl p-4 mb-6 text-left space-y-3">
                    <div className="flex justify-between items-center"><span className="text-white/60">Utilisateur :</span><span className="text-white font-medium">{resetResult.user_name}</span></div>
                    <div className="flex justify-between items-center"><span className="text-white/60">Téléphone :</span><span className="text-white">{resetResult.user_phone}</span></div>
                    <div className="flex justify-between items-center pt-2 border-t border-white/10"><span className="text-white/60">Nouvelle clé privée :</span><div className="flex items-center gap-2"><code className="text-orange-400 bg-black/30 px-2 py-1 rounded font-mono text-sm">{resetResult.new_private_key}</code><button onClick={copyNewKey} className="text-primary-400"><FaCopy /></button></div></div>
                  </div>
                  <p className="text-yellow-400 text-sm mb-4">⚠️ Cette clé est indispensable pour se connecter en cas de perte de mot de passe. Veuillez la communiquer à l'utilisateur.</p>
                  <button onClick={closeResetModal} className="btn-primary w-full">Fermer</button>
                </div>
              ) : (
                <div><p className="text-gray-300 mb-4">Êtes-vous sûr de vouloir réinitialiser la clé privée de <span className="font-bold text-white">{selectedUserForReset?.name}</span> ?</p><p className="text-yellow-400 text-sm mb-6">⚠️ Cette action est irréversible. L'ancienne clé ne fonctionnera plus.</p><div className="flex gap-3"><button onClick={closeResetModal} className="flex-1 px-4 py-2 rounded-lg bg-gray-700 text-white">Annuler</button><button onClick={confirmResetKey} disabled={resettingKey} className="flex-1 px-4 py-2 rounded-lg bg-orange-600 text-white flex items-center justify-center gap-2">{resettingKey ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : <><FaSync /> Confirmer</>}</button></div></div>
              )}
            </div>
          </div>
        </div>
